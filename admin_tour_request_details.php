...
$mark_read = isset($_REQUEST['mark_read']) ? (int)$_REQUEST['mark_read'] : 0;
...
if ($mark_read === 1 && (int)($req['is_read_by_agent'] ?? 1) === 0) {
    $upd = $conn->prepare("UPDATE tour_requests SET is_read_by_agent = 1 WHERE tour_id = ?");
  ...
  " . ($status === 'Expired'
            ? "<div class='section-divider'></div>
               <div class='detail-card full-width'>
                 ...
                 <div class='detail-card-content'>
                   <div class='reason-box' style='border-left-color: #9ca3af;'>This tour request expired automatically because the scheduled date and time passed without a response. The client has been notified via email.</div>
                   " . ($expiredAt
