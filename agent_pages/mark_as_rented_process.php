...
try {
    ...
    // ----- Verify property ownership & eligibility -----
    $prop_stmt = $conn->prepare("
        SELECT p.property_ID, p.StreetAddress, p.City, p.Status, p.listing_type, p.approval_status
        FROM property p
        INNER JOIN property_log pl ON p.property_ID = pl.property_id
        WHERE p.property_ID = ? AND pl.account_id = ? AND pl.action = 'CREATED'
        ORDER BY pl.log_timestamp DESC LIMIT 1
    ");
    $prop_stmt->bind_param("ii", $property_id, $agent_id);
    $prop_stmt->execute();
    ...
    if ($property['listing_type'] !== 'For Rent') {
        respond_json(false, 'This property is not a rental listing.');
    }
    if ($property['approval_status'] !== 'approved') {
        respond_json(false, 'Property must be approved before it can be rented.');
    }
    if (!in_array($property['Status'], ['For Rent'])) {
        respond_json(false, 'This property is not available for rental verification. Current status: ' . $property['Status']);
    }
    ...
    $upload_base = realpath(__DIR__ . '/..') . '/rental_documents';
    ...
    for ($i = 0; $i < count($files['name']); $i++) {
        ...
        // Store relative path from project root for DB
    ...
    try {
        ...
        if (!$ins->execute()) {
            throw new Exception('Failed to insert rental verification.');
        }
        ...
        // 4. Property log
        $log_msg = 'Rental verification submitted by agent';
        $log_stmt = $conn->prepare("
            INSERT INTO property_log (property_id, account_id, action, reason_message, reference_id)
            VALUES (?, ?, 'UPDATED', ?, ?)
        ");
        $log_stmt->bind_param("iisi", $property_id, $agent_id, $log_msg, $verification_id);
        $log_stmt->execute();
        $log_stmt->close();
        ...
        $notif_action = 'admin_rental_approvals.php';
        $notif_stmt = $conn->prepare("
            INSERT INTO notifications (item_id, item_type, title, message, category, priority, action_url, icon, is_read, created_at)
            VALUES (?, 'property_rental', ?, ?, 'update', 'high', ?, 'bi-house-check', 0, NOW())
        ");
        $notif_stmt->bind_param("isss", $verification_id, $notif_title, $notif_msg, $notif_action);
        if (!$notif_stmt->execute()) {
            throw new Exception('Failed to create admin notification: ' . $notif_stmt->error);
        }
        $notif_stmt->close();

        $conn->commit();

        respond_json(true, 'Rental verification submitted successfully! Your submission is pending admin review.', [
        ...
        foreach ($uploaded_files as $f) {
            $cleanup_path = isset($f['full_path']) ? $f['full_path'] : $f['file_path'];
    ...
    error_log("Rental verification error: " . $e->getMessage());
